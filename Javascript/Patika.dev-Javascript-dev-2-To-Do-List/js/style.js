...
let savedData = localStorage.getItem("toDOList") ? localStorage.getItem("toDOList") : ""
if(savedData!=""){
    ulDom.innerHTML=savedData
}
...
element.classList.remove("checked")
    ...
    title: 'Uyarı - Kayıt Kalıcı Siliniyor',
    content: `Silinen Kayıt :  ${element.parentElement.innerText.slice(0,-1)}`,
    buttons: [
        {
            text: 'Geri Al',
            class: 'btn btn-sm btn-primary',
            handler: () => {
                addElement(e.target.parentElement.innerText.slice(0,-1),e.target.parentElement.className)
                toastContainer.innerHTML=""
            }
        },
        {
...
localStorage.setItem("toDOList",ulDom.innerHTML)
})

function addElement(element,checked=""){
if(checked=="checked") checked="class='checked'"
ulDom.innerHTML+=(`<li ${checked}>${element}<span id="delete-li" class="close" >x</span></li>`)
localStorage.setItem("toDOList",ulDom.innerHTML)
bs5Utils.Snack.show('success', 'Listeye Ekleme Yapıldı', 3000, true);
}
...
task.value=""
bs5Utils.Snack.show('danger', 'Listeye Boş Ekleme Yapamazsınız!', 3000, true);
}
}